...
$sel = mysqli_query($mysqli,"select count(*) as allcount from expenses where status IS NULL ");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of record with filtering
$sel = mysqli_query($mysqli,"select count(*) as allcount from expenses WHERE status IS NULL AND 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from expenses WHERE status IS NULL AND 1 ".$searchQuery." order by expdate DESC limit ".$row.",".$rowperpage;
