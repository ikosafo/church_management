...
$sel = mysqli_query($mysqli,"select count(*) as allcount from logs WHERE `user` != 'ikosafo'");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of record with filtering
$sel = mysqli_query($mysqli,"select count(*) as allcount from logs WHERE `user` != 'ikosafo' AND 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from logs WHERE `user` != 'ikosafo' AND 1 ".$searchQuery." order by logdate DESC limit ".$row.",".$rowperpage;
