...
if ($branch == "All") {
    ...
    $sel = mysqli_query($mysqli, "select count(*) as allcount from `visitor`");
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];

    ## Total number of record with filtering
    $sel = mysqli_query($mysqli, "select count(*) as allcount from `visitor` WHERE 1 " . $searchQuery);
    $records = mysqli_fetch_assoc($sel);
    $totalRecordwithFilter = $records['allcount'];

    ## Fetch records
    $empQuery = "select * from `visitor` WHERE 1 " . $searchQuery . " order by full_name limit " . $row . "," . $rowperpage;
