...
$pinq = $mysqli->query("select * from website_dwelfare ORDER BY id DESC");
?>
<style>
    .dataTables_filter {
        display: none;
                ...
                while ($fetch = $pinq->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><?php $imageid = $fetch['imageid'];
                            $getid = $mysqli->query("select * from website_image_dwelfare where imageid = '$imageid'");
                        ...
                        <td>
                            <button type="button"
                                    data-type="confirm"
                                    class="btn btn-danger delete_dwelfare"
...
<script>
    ...
    $(document).off('click', '.delete_dwelfare').on('click', '.delete_dwelfare', function () {
        var theindex = $(this).attr('i_index');
        //alert(theindex)
        $.confirm({
            title: 'Delete Welfare Dept. Content!',
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "ajax/queries/delete_dwelfare.php",
                            data: {
                                i_index: theindex
                            },
                            dataType: "html",
                            success: function (text) {
                                $.ajax({
                                    url: "ajax/tables/dwelfare_table.php",
                                    beforeSend: function () {
                                        KTApp.blockPage({
                                            overlayColor: "#000000",
                                            type: "v2",
