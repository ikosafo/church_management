...
$pinq = $mysqli->query("select * from website_myouth ORDER BY id DESC");
?>
<style>
    .dataTables_filter {
        display: none;
                ...
                while ($fetch = $pinq->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><?php $imageid = $fetch['imageid'];
                            $getid = $mysqli->query("select * from website_image_myouth where imageid = '$imageid'");
                        ...
                        <td>
                            <button type="button"
                                    data-type="confirm"
                                    class="btn btn-danger delete_myouth"
...
<script>
    ...
    $(document).off('click', '.delete_myouth').on('click', '.delete_myouth', function () {
        var theindex = $(this).attr('i_index');
        //alert(theindex)
        $.confirm({
            title: 'Delete Youth Ministry Content!',
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "ajax/queries/delete_myouth.php",
                            data: {
                                i_index: theindex
                            },
                            dataType: "html",
                            success: function (text) {
                                $.ajax({
                                    url: "ajax/tables/myouth_table.php",
                                    beforeSend: function () {
                                        KTApp.blockPage({
                                            overlayColor: "#000000",
                                            type: "v2",
