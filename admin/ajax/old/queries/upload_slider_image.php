...
$chk = $mysqli->query("select * from website_image_slider where imageid = '$randno'");
$count = mysqli_num_rows($chk);

if ($count == "0"){
    $insertfile  = $mysqli->query("INSERT INTO `website_image_slider`
            (`image_name`,
             `image_location`,
             `image_size`,
             `image_type`,
...
VALUES ('$filename',
    ...
    $query = $mysqli->query("select * from website_image_slider where applicant_id = '$randno'");
    ...
    $updatefile  = $mysqli->query("UPDATE `website_image_slider`
SET
  `image_name` = '$filename',
