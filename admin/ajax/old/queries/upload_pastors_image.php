...
$chk = $mysqli->query("select * from website_image_pastors where imageid = '$randno'");
$count = mysqli_num_rows($chk);

if ($count == "0"){
    $insertfile  = $mysqli->query("INSERT INTO `website_image_pastors`
            (`image_name`,
             `image_location`,
             `image_size`,
             `image_type`,
    ...
    $query = $mysqli->query("select * from website_image_pastors where imageid = '$randno'");
    $res = $query->fetch_assoc();
    $filename2 =  $res['image_location'];
    $use = substr($filename2,strpos($filename2,"/")+1);
    unlink("../../../uploads/".$use);

    $updatefile  = $mysqli->query("UPDATE `website_image_pastors`
        SET
          `image_name` = '$filename',
