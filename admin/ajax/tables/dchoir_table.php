...
$pinq = $mysqli->query("select * from website_dchoir ORDER BY id DESC");
?>
<style>
    .dataTables_filter {
        display: none;
                ...
                while ($fetch = $pinq->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><?php $imageid = $fetch['imageid'];
                            $getid = $mysqli->query("select * from website_image_dchoir where imageid = '$imageid'");
                        ...
                        <td>
                            <button type="button"
                                    data-type="confirm"
                                    class="btn btn-danger delete_dchoir"
...
<script>
    ...
    $(document).off('click', '.delete_dchoir').on('click', '.delete_dchoir', function () {
        var theindex = $(this).attr('i_index');
        //alert(theindex)
        $.confirm({
            title: 'Delete Choir Dept. Content!',
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "ajax/queries/delete_dchoir.php",
                            data: {
                                i_index: theindex
                            },
                            dataType: "html",
                            success: function (text) {
                                $.ajax({
                                    url: "ajax/tables/dchoir_table.php",
                                    beforeSend: function () {
                                        KTApp.blockPage({
                                            overlayColor: "#000000",
                                            type: "v2",
