...
$pinq = $mysqli->query("select * from website_dprayer ORDER BY id DESC");
?>
<style>
    .dataTables_filter {
        display: none;
                ...
                while ($fetch = $pinq->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><?php $imageid = $fetch['imageid'];
                            $getid = $mysqli->query("select * from website_image_dprayer where imageid = '$imageid'");
                        ...
                        <td>
                            <button type="button"
                                    data-type="confirm"
                                    class="btn btn-danger delete_dprayer"
...
<script>
    ...
    $(document).off('click', '.delete_dprayer').on('click', '.delete_dprayer', function () {
        var theindex = $(this).attr('i_index');
        //alert(theindex)
        $.confirm({
            title: 'Delete Prayer Dept. Content!',
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "ajax/queries/delete_dprayer.php",
                            data: {
                                i_index: theindex
                            },
                            dataType: "html",
                            success: function (text) {
                                $.ajax({
                                    url: "ajax/tables/dprayer_table.php",
                                    beforeSend: function () {
                                        KTApp.blockPage({
                                            overlayColor: "#000000",
                                            type: "v2",
